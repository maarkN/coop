...
| **Hash Bank for Reported Media**         | Media from each CyberTip accepted by NCMEC is added to this hash bank so it can be matched if it is uploaded again. Each item is retried a few times; if it still fails, the submission is marked as failed. Only applies to [production submissions](#test-vs-production-submissions). |
